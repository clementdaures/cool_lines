...
'mel mirror expression : DPK_mirrorGeo -so "DPK_bcs1" -ax X -b 0 -r -lr;'
...
class mainWidget(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        #####################################
        #Creation
        """self.create_edge_line_title = QLabel(self)
        self.create_edge_line_title.setText('Create Line from Edge')"""
        ...
        """self.create_paint_line_title = QLabel(self)
        self.create_paint_line_title.setText('Paint on Mesh')"""
        ...
        #####################################
        #Organizing:
        """self.main_layout.addWidget(self.create_edge_line_title)"""
        ...
        """self.main_layout.addWidget(self.create_paint_line_title)"""
    ...
    def createLineFromEdge(self):
        ...
        #shading_group = detectOrCreateShader()
        ...
        #cmds.sets(result_mesh, e=True, forceElement=shading_group)
        ...
        #cmds.hyperShade(assign='outline_preview_shader', geo='result_mesh')
    ...
    def paintLineOnMesh(self):
        ...
        #https://www.regnareb.com/pro/2014/05/wait-for-user-input-in-maya/
        try:
...
class lineListDisplayWidget(QWidget):
    def __init__(self, parent_dialog, stroke_name, *args,**kwargs):
        ...
        self.disable_line_checkbox = QuteCheckbox(self)
...
def executed_after_action(start_scale):
    ...
    if cmds.objectType(shape) == 'stroke':
        ...
        #myWindowInstance.main_widget.refreshList()
...
if __name__ == '__main__':
    ...
    myWindowInstance = MyWindow()
    myWindowInstance.show(dockable=True)

    """if not QApplication.instance():

        app_start = 1

        app = QApplication(sys.argv) #if no apllication executed create aplication

    else:

        app_start = 0

        app = QApplication.instance() #if apllication executed create intance

    window = MainWindow()
    window.show()
    #app.setStyleSheet(ph_theme)
